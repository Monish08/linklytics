...
import { Badge } from '@/components/ui/badge'  // For preview
...
const Success = () => {
  ...
  const [includePwInShare, setIncludePwInShare] = useState(true)  // Checked = include pw (direct)
  const [loading, setLoading] = useState(true)

  // Refetch link on toggle change
  useEffect(() => {
    if (!loading) {
      fetchShortLink()
    }
  ...
  const fetchShortLink = async () => {
    try {
      const { data } = await urlsAPI.analytics(shortCode)
      let link = `http://localhost:5000/api/urls/${shortCode}`  // Prod: env var
    ...
    <div className="flex min-h-[80vh] items-center justify-center p-4">
      <Card className="w-full max-w-lg">
        <CardHeader className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">Here's your linklytic:</p>
          {password && (
            <div className="flex items-center justify-center gap-2 text-xs text-destructive">
              <Lock className="h-3 w-3" />
              <span>Protected—users will need password to access.</span>
            </div>
          )}
        </CardHeader>
        <CardContent className="space-y-6 p-6">
          ...
          {/* Redesigned Toggle (if protected) */}
{password && (
  <div className="bg-muted p-3 rounded-lg space-y-3">
    <div className="flex flex-col items-center gap-2">
      ...
      <Switch 
        ...
        onCheckedChange={togglePwInShare} 
        className="data-[state=checked]:bg-green-600"
      />
    </div>
    <Badge 
      variant="secondary" 
      className="text-xs w-full text-center cursor-pointer hover:underline" 
      onClick={openLink}
    >
      Link preview: {shortLink.replace('http://localhost:5000/api/urls/', '').replace('?pw=...', '?pw=*')}
    </Badge>
  </div>
)}
          ...
          <div className="flex flex-col sm:flex-row gap-2 justify-center">
            <Button onClick={copyLink} variant="outline" className="flex items-center gap-2 justify-center">
              <Copy className="h-4 w-4" />
              Copy Linklytic
            </Button>
            <Button onClick={downloadQR} variant="outline" className="flex items-center gap-2 justify-center">
              <Download className="h-4 w-4" />
              Download QR
            </Button>
            <Button onClick={shareWhatsApp} variant="outline" className="flex items-center gap-2 justify-center">
          ...
          {password && !includePwInShare && (
            <p className="text-xs text-destructive text-center">
              Note: Shared link is plain—recipients will see password prompt.
            </p>
          )}
        </CardContent>
      </Card>
